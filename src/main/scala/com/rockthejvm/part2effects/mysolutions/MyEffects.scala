...
object MyEffects {
  ...
  // example: print to the console return value is same but effect isn't identical
  val printSmt: Unit = println("Cats effects")
  val printSmt_v2: Unit = ()
   ...
   */
//bad effect because of the last thing
...
//    def flatMap[B](f: A => MyIO[B]): MyIO[B] = f(unsafeRun())
    def flatMap[B](f: A => MyIO[B]): MyIO[B] = MyIO(() => f(unsafeRun()).unsafeRun())
  }

  val anIntIO = MyIO[Int](() => {
...
//  2
  def measure[A](computation: MyIO[A]): MyIO[Long] = {
    //first attempt
//    val start: Long = currentTime.unsafeRun()
//    computation.map(a => currentTime.unsafeRun() - start)
//rewrote
    for {
      start <- currentTime
      _ <- computation
 ...
 */
  ...
  def printing(input: String): MyIO[Unit] = MyIO[Unit] (() => println(input))

//4
  val reader: MyIO[String] = MyIO[String](() => StdIn.readLine())

  def testComposed = {
    for {
      line1 <- reader
      line2 <- reader
      _ <- printing(s"$line1 and $line2 were entered")
    } yield ()
  }
...
//    import cats.instances.long._
    import cats.syntax.show._
//    val result = measure(printing("CAAAAAAAAAAAT")).unsafeRun()
    val result = measure(MyIO(() => sleep(1000))).unsafeRun()
    println(result)
//    println("Waiting for input")
//    val read = reader.unsafeRun()
//    println(s"Input was: $read")
    testComposed.unsafeRun()
  }
}
